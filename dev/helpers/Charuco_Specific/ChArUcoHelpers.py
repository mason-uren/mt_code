...
def get_pose(markers,ids,chorners,chids,board,intrinsics,dist,debug_verbose):
    ...
    mean_error = charuco_reprojection_error(board,chorners,chids,rvec,tvec,intrinsics,dist)
    ...
    if(debug_verbose == True):
        ...
        if(extrinsics_4x4 is None):
            ...
            print("TVEC: " + str(tvec))
            print("RVEC: " + str(rvec))
            print("RMS ERROR: " + str(mean_error))
            print("\n")
...
def charuco_reprojection_error(board,corners,ids,rvec,tvec,intrinsics,dist,markers=False):
    ...
    # else:
    error = charuco_reprojection_error_for_every_corner(board, corners, ids, rvec, tvec, intrinsics, dist)
...
def generate_reprojection_error_quiver_plot_markers(image,aruco_dict,board,rvec,tvec,intrinsics,dist,exageration_factor = 8,QUIVER=True):
...
def generate_reprojection_error_quiver_plot_ids(image,aruco_dict,board,rvec,tvec,intrinsics,dist,exageration_factor = 30):
    # board positions
    image_copied = cv2.blur(image,(3,3))
    image_copied = cv2.cvtColor(image_copied,cv2.COLOR_GRAY2BGR)
    ...
    reprojection_error = charuco_reprojection_error_for_every_corner(board_aruco, chcorners, chids, rvec, tvec, intrinsics, dist)
